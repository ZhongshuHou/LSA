...
class ASqBi(nn.Module):
    ...
    def get_bias(self, mmf):
        F = mmf.shape[-1]
        bias = (-1)*(toeplitz(range(0,F))**2).astype('float32')
        bias = torch.from_numpy(bias).to(mmf.device)
    ...
    def forward(self,x):
        ...
        F = x.shape[3]
        if C < self.heads:
            qf = x.permute(0,2,3,1)
            v = qf
            kf = x.permute(0,2,1,3)
            mmf = torch.matmul(qf,kf) / math.sqrt(C*F/2)
            mmf = torch.softmax(mmf, dim=-1)
            mmf = torch.matmul(mmf,v) #shape = (Bs, T, F, C)
        else:
            head_list = []
            Ci = C // self.heads
            for head in range(self.heads):
                qf = x[:,head*Ci:(head+1)*Ci,:,:].permute(0,2,3,1)
                v = qf
                kf = x[:,head*Ci:(head+1)*Ci,:,:].permute(0,2,1,3)
                mmf = torch.matmul(qf,kf) / math.sqrt(Ci*F/2)
                bias = self.get_bias(mmf)
                mmf = torch.softmax(mmf + bias * (self.factor**(-(head+1))), dim=-1)
                mmf = torch.matmul(mmf,v) #shape = (Bs, T, F, C)
                head_list.append(mmf)
            mmf = torch.cat(head_list, dim=-1)
