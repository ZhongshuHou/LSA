...
class FreqLocalMask(nn.Module):
    ...
    def forward(self, inp):
        ...
        # inp.shape = (bs, F, T)
        batch_size = inp.shape[0]
        freq_dim = inp.shape[2]
        local_num = int(np.sqrt(freq_dim))
        causal_mask = self.lower_triangular_mask([1,freq_dim,freq_dim], local_num=local_num)
        unmasked = torch.zeros([batch_size, freq_dim, freq_dim])
        masked = torch.fill_(torch.empty([batch_size, freq_dim, freq_dim]), self.mask_value)
        att_mask = torch.where(causal_mask, unmasked, masked)
        return att_mask
...
class ASA(nn.Module):
    ...
    def forward(self,x):
        ...
        qf = x1.permute(0,2,3,1)#shape = (Bs, T, F, C)
        v = x1.permute(0,2,3,1)#shape = (Bs, T, F, C)
        kf = x1.permute(0,2,1,3)#shape = (Bs, T, C, F)
        mmf = torch.matmul(qf,kf) / math.sqrt(C*F/2)#shape = (Bs, T, F, F)
        local_mask = FreqLocalMask()(mmf).to(mmf.device)#shape = (Bs,F, F)
        local_mask = local_mask.view(local_mask.shape[0],1, local_mask.shape[2],local_mask.shape[2])   #shape = (Bs,1, F, F)
        mmf = torch.softmax(mmf + local_mask, dim=-1) #shape = (Bs, T, F, F)
...
class Encoder(nn.Module):
    def __init__(self, in_channel, filter_size, kernel_size, strides, paddings, **kwargs):
            ...
            if i == 0:
                ...
                self.conv_list.append(
                    nn.Conv2d(in_channels=self.filter_size[i-1],
                              ...
                              kernel_size=self.kernel_size[i],
                              stride=self.strides[i]))
            self.padding_list.append(nn.ZeroPad2d([paddings[i][0],paddings[i][1],0,0]))
