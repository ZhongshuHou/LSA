...
The modified method is combined with MTFAA [MTFAA_Net_full_F_ASqbi.py](https://github.com/ZhongshuHou/LSA/blob/main/MTFAA_Net_full_F_ASqbi.py) and the ablation test results can be found in the last row of table below. It can be seen that the overall performance degrades compared with LSA. It may be explained that the negative bias added to local attention region weakens the model capability to extract local intercorrelation.
